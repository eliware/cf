...
const pickerAsset = name => readFileSync(fileURLToPath(new URL(`./${name}`, import.meta.url)), 'utf8');
const pickerHtml = () => pickerAsset('oauth-web/oauth-picker.html')
  .replace('__CF_SCOPE_MODEL__', JSON.stringify({ categories: SCOPE_CATALOG_DATA, modules: MODULE_CATALOG }).replace(/</g, '\\u003c'))
  .replaceAll('__COPYRIGHT_YEAR__', `2026${new Date().getUTCFullYear() > 2026 ? `-${new Date().getUTCFullYear()}` : ''}`)
...
export async function loginOAuth({ clientId, scopes = DEFAULT_OAUTH_SCOPES, scopePicker = false, ports = DEFAULT_OAUTH_PORTS, bindHost = '127.0.0.1', redirectHost = '127.0.0.1', fetchImpl = fetch, open = openBrowser, print = console.log, serverFactory = http.createServer }) {
  ...
  const callback = new Promise((resolve, reject) => server.on('request', (request, response) => {
    ...
    if (scopePicker && url.pathname === '/' && request.method === 'GET') { response.writeHead(200, { 'content-type': 'text/html; charset=utf-8' }); response.end(pickerHtml()); return; }
    if (scopePicker && url.pathname === '/oauth-picker.css' && request.method === 'GET') { response.writeHead(200, { 'content-type': 'text/css; charset=utf-8' }); response.end(pickerAsset('oauth-web/oauth-picker.css')); return; }
    if (scopePicker && url.pathname === '/oauth-picker.mjs' && request.method === 'GET') { response.writeHead(200, { 'content-type': 'text/javascript; charset=utf-8' }); response.end(pickerAsset('oauth-web/oauth-picker.mjs')); return; }
    if (scopePicker && url.pathname === '/oauth-web/cloudflare-115x53.png' && request.method === 'GET') { response.writeHead(200, { 'content-type': 'image/png' }); response.end(readFileSync(fileURLToPath(new URL('./oauth-web/cloudflare-115x53.png', import.meta.url)))); return; }
    if (scopePicker && url.pathname === '/oauth-web/eliware-115x115.png' && request.method === 'GET') { response.writeHead(200, { 'content-type': 'image/png' }); response.end(readFileSync(fileURLToPath(new URL('./oauth-web/eliware-115x115.png', import.meta.url)))); return; }
