...
export async function handleZoneSettings({ cf, action, opts, body, outputJson, toJsonOutput, fail }) {
  ...
  if (action === 'set') {
    ...
    if (opts['dry-run']) return console.log(JSON.stringify({ zoneId, setting, value: body.value, dryRun: true }, null, 2));
