...
export async function handleDnsRecords({ cf, action, opts, body, outputJson, toJsonOutput, fail }) {
  ...
  if (action === 'list') {
    ...
    return outputJson ? toJsonOutput(items) : items.forEach(r => console.log(`${r.id} ${r.type} ${r.name} ${r.content}`));
  }
  ...
  if (action === 'create') {
    ...
    if (opts['dry-run']) return console.log(JSON.stringify({ zoneId, action: 'create', body, dryRun: true }, null, 2));
  ...
  if (action === 'update') {
    requireValue(id, 'Missing --id', fail);
    requireValue(body, 'Missing --data or --file', fail);
    if (opts['dry-run']) return console.log(JSON.stringify({ zoneId, id, action: 'update', body, dryRun: true }, null, 2));
