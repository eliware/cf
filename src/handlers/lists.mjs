...
export async function handleLists({ cf, action, opts, outputJson, toJsonOutput, fail }) {
  ...
  if (action === 'list') {
    ...
    return outputJson ? toJsonOutput(items) : items.forEach(l => console.log(`${l.id} ${l.name}`));
  }
