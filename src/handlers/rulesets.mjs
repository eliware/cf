...
export async function handleRulesets({ cf, action, opts, body, outputJson, toJsonOutput, fail }) {
  ...
  if (action === 'list') {
    ...
    return outputJson ? toJsonOutput(target) : console.log(JSON.stringify(target, null, 2));
  }
  ...
  if (action === 'create') {
    ...
    if (opts['dry-run']) return console.log(JSON.stringify({ zoneId, accountId, action: 'create', body, dryRun: true }, null, 2));
  ...
  if (action === 'update') {
    requireValue(id, 'Missing --id', fail);
    requireValue(body, 'Missing --data or --file', fail);
    if (opts['dry-run']) return console.log(JSON.stringify({ zoneId, accountId, id, action: 'update', body, dryRun: true }, null, 2));
