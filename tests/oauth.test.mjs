...
test('OAuth scope picker opens a local setup page and redirects with selected scopes', async () => {
  ...
  const landing = new URL(printed[0].split('\n')[1]);
   await new Promise((resolve, reject) => http.get(landing, response => { let body = ''; response.on('data', chunk => { body += chunk; }); response.on('end', () => { expect(body).toContain('Set up cf'); expect(body).toContain('scope-search'); resolve(); }); }).on('error', reject));
  let authorization;
